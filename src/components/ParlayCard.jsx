    ...
    <div className="flex items-center gap-4 rounded-xl px-5 py-4"
      ...
      <div className="shrink-0 text-right">
        <p className="font-mono text-base font-black text-white">
          <OddsValue decimal={leg.decimalOdds} />
        </p>
        <p className="mt-0.5 font-mono text-[10px] font-bold" style={{ color: "#10B981" }}>
          +{leg.ev}% EV
        </p>
      </div>
    </div>
      ...
      <div className="flex flex-col items-center justify-center gap-4 rounded-2xl p-6 lg:rounded-xl"
        ...
        <div className="text-center">
          <p className="font-mono text-[10px] font-bold uppercase tracking-widest mb-1"
            style={{ color: "rgba(245,158,11,0.6)" }}>Combined Odds</p>
          <p className="font-display font-black leading-none"
            style={{ fontSize: "clamp(2.5rem,7vw,3.5rem)", color: "#F59E0B", textShadow: "0 0 30px rgba(245,158,11,0.35)" }}>
            {data.combinedOdds}x
          </p>
        ...
        <div className="w-full space-y-2">
          <div className="flex items-center justify-between rounded-lg px-3 py-2"
            ...
            <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "rgba(16,185,129,0.7)" }}>AI Edge</span>
            <span className="font-mono text-sm font-black" style={{ color: "#10B981" }}>
              {data.combinedEV >= 0 ? "+" : ""}{data.combinedEV}%
            </span>
          </div>
          <div className="flex items-center justify-between rounded-lg px-3 py-2"
            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
            <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.35)" }}>True Prob</span>
            <span className="font-mono text-sm font-bold text-white">{data.combinedTrueProb}%</span>
          </div>
          <div className="flex items-center justify-between rounded-lg px-3 py-2"
            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
            <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.35)" }}>Legs</span>
            <span className="font-mono text-sm font-bold text-white">{data.legCount}</span>
          </div>
        </div>
      </div>
    </div>
...
export default function ParlayCard({ parlay, loading, isPro, onUnlock }) {
  ...
  // Ghost legs shown blurred behind the lock for non-pro users
  const ghostLegs = [
    ...
    <div className="overflow-hidden rounded-2xl"
      ...
      <div className="relative p-6 sm:p-8">
        {isPro ? (
          ...
          <>
            {/* Blurred ghost content behind lock */}
